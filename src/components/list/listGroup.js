...
export default class ListGroup extends React.Component {
  ...
  renderListSectionHeader() {
    ...
    if (lists && activeList) {
      ...
      let date = list.type === 'today' ? Moment().format('dddd MMM Do') : null;
  ...
  renderWeek() {
    const {items, sorts, nowEditing, nowDragging, add, remove, done, reorder, edit, onEdit, activeList}  = this.props;

    let days = []
    for (var i = 1; i < 8; i++) {
      let day = Moment().add(i, 'day');
      days.push({'day': day.format('dddd'),'date': day.format('MMM Do')})
    }
  ...
  render() {
    ...
    let itemOrder = sorts[activeList];
    let completedItems = []
